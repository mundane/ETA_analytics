StartTest(function (t) {
    ...
    t.click(nameBox, function() {
        nameBox.focus();
        t.type(nameBox.inputEl, 'Joe Schmoe', function () {
            t.is(nameBox.getValue(), "Joe Schmoe", 'Correct text found in name field');

            ageBox.focus();
            t.type(ageBox, '123[UP]', function () {
                t.is(ageBox.getValue(), "124", 'Correct text found in age field');

                emailBox.focus();
                t.type(emailBox, 'incorrect@email', function () {
                    t.is(emailBox.getValue(), "incorrect@email", 'Correct text found in email field');

                    passBox.focus();
                    t.type(passBox, 'secret', function () {

                        pass2Box.focus();
                        t.type(pass2Box, 'secret[ENTER]', function () {
