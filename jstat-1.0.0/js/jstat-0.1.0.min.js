...
jstat.plot = function (a, b, c) {
    ...
    } else for (var f = 0; f < a.length; f++) e.push([a[f], b[f]]);
    a = "jstat graph";
    if (c != null) {
        if (c.type != null) if (c.type == "l") d.series.lines.show = true;
        else if (c.type == "p") {
            d.series.lines.show = false;
            d.series.points.show = true
        }
        if (c.hover != null) d.grid = {
            hoverable: c.hover
        };
...
jstat.stirlerr = function (a) {
    ...
    if (a > 35) return (0.08333333333333333 - (0.002777777777777778 - (7.936507936507937E-4 - 5.952380952380953E-4 / c) / c) / c) / a;
...
jstat.lgamma = function (a) {
    return function (b) {
        ...
        if (b > 0) return b > 1E17 ? b * (Math.log(b) - 1) : b > 4934720 ? jstat.LN_SQRT_2PI + (b - 0.5) * Math.log(b) - b : jstat.LN_SQRT_2PI + (b - 0.5) * Math.log(b) - b + jstat.lgammacor(b);
...
jstat.gamma = function (a) {
    ...
    if (g < jstat.DBL_EPSILON) if (g >= jstat.DBL_MIN) i = 1 / g;
    else return Number.POSITIVE_INFINITY;
    else if (g < 12) {
        n = g;
        if (g < 1) {
...
jstat.incompleteBeta = function (a, b, c) {
    function d(f, h, i) {
        ...
        if (g > 100) {
            console.warn("a or b too big, or MAXIT too small in betacf: " + f + ", " + h + ", " + i + ", " + o);
            return o
        }
...
jstat.log1p = function (a) {
    ...
    for (var c = 1; c < 50; c++) if (c % 2 === 0) b -= Math.pow(a, c) / c;
...
jstat.logBeta = function (a, b) {
    ...
    if (d >= 10) {
        ...
        return jstat.lgamma(d) + c + d - d * Math.log(d + e) + (e - 0.5) * jstat.log1p(-d / (d + e))
...
jstat.dbinom_raw = function (a, b, c, d, e) {
    ...
    a = Math.log(jstat.TWO_PI) + Math.log(a) + jstat.log1p(-a / b);
    return e ? c - 0.5 * a : Math.exp(c - 0.5 * a)
};
jstat.max = function (a) {
    for (var b = Number.NEGATIVE_INFINITY, c = 0; c < a.length; c++) if (a[c] > b) b = a[c];
    return b
};
Range = Class.extend({
    init: function (a, b, c) {
...
var ContinuousDistribution = Class.extend({
    ...
    density: function (a) {
        if (isNaN(a)) if (Range.validate(a)) {
    ...
    NormalDistribution = ContinuousDistribution.extend({
        ...
        _pdf: function (a, b) {
            ...
            if (c <= 0) {
                if (c < 0) throw "invalid sigma in _pdf";
        ...
        _cdf: function (a, b, c) {
            ...
            a = function (i, g, l, n, p) {
                ...
                } else if (p && w < 1.0E170 || x && -37.5193 < i && i < 8.2924 || n && -8.2924 < i && i < 37.5193) {
                    m = 1 / (i * i);
                    u = q[5] * m;
                    r = m;
                    for (s = 0; s < 4; ++s) {
    ...
    GammaDistribution = ContinuousDistribution.extend({
        ...
        _pdf: function (a, b) {
            ...
            if (a < 0) return b ? Number.NEGATIVE_INFINITY : 0;
            if (d == 0) return a == 0 ? Number.POSITIVE_INFINITY : b ? Number.NEGATIVE_INFINITY : 0;
            if (a == 0) {
    ...
    BetaDistribution = ContinuousDistribution.extend({
        ...
        _cdf: function (a, b, c) {
            ...
            if (a <= 0) return b ? c ? Number.NEGATIVE_INFINITY : 0 : c ? 0.1 : 1;
    ...
    StudentTDistribution = ContinuousDistribution.extend({
        ...
        _pt: function (a, b, c) {
            ...
            if (e > 1.0E100) {
                ...
                b = new BetaDistribution(0.5, d / 2);
        ...
        _pnt: function (a, b, c) {
            ...
            h = jstat.fmin2(i, 1);
            return b ? c ? Math.log(h) : h : c ? jstat.log1p(-h) : 0.5 - h + 0.5
        },
    ...
    DistributionPlot = Plot.extend({
        ...
        setType: function (a) {
            ...
            if (this._options.series == null) this._options.series = {
                ...
                else this._options.series.lines.show = a.show;
                if (this._options.series.points == null) this._options.series.points = b;
        ...
        render: function () {
            if (this._distribution != null) if (this._showPDF && this._showCDF) {
                this.setData([{
                    yaxis: 1,
...
DistributionFactory.build = function (a) {
    if (a.NormalDistribution) if (a.NormalDistribution.mean != null && a.NormalDistribution.standardDeviation != null) return new NormalDistribution(a.NormalDistribution.mean[0], a.NormalDistribution.standardDeviation[0]);
    else throw "Malformed JSON provided to DistributionBuilder " + a;
    else if (a.LogNormalDistribution) if (a.LogNormalDistribution.location != null && a.LogNormalDistribution.scale != null) return new LogNormalDistribution(a.LogNormalDistribution.location[0], a.LogNormalDistribution.scale[0]);
    else throw "Malformed JSON provided to DistributionBuilder " + a;
    else if (a.BetaDistribution) if (a.BetaDistribution.alpha != null && a.BetaDistribution.beta != null) return new BetaDistribution(a.BetaDistribution.alpha[0], a.BetaDistribution.beta[0]);
    else throw "Malformed JSON provided to DistributionBuilder " + a;
    else if (a.GammaDistribution) if (a.GammaDistribution.shape != null && a.GammaDistribution.scale != null) return new GammaDistribution(a.GammaDistribution.shape[0], a.GammaDistribution.scale[0]);
    else throw "Malformed JSON provided to DistributionBuilder " + a;
    else if (a.StudentTDistribution) if (a.StudentTDistribution.degreesOfFreedom != null && a.StudentTDistribution.nonCentralityParameter != null) return new StudentTDistribution(a.StudentTDistribution.degreesOfFreedom[0], a.StudentTDistribution.nonCentralityParameter[0]);
